...
#import sys
#sys.path.insert(0, 'Sql/Database_Table.py')
from Database_Table import db_execute_fetch

st.set_page_config(page_title="Day 5", layout="wide")

def loadData():
    query = "select * from Sentiment_Analysis_Table"
...
def stBarChart():
    df = loadData()
    dfCount = pd.DataFrame({'Tweet_count': df.groupby(['hashtags'])['full_text'].count()}).reset_index()
    ...
    barChart(dfCount.head(num), title, "hashtags", "hashtags")


def langPie():
    df = loadData()
    dfLangCount = pd.DataFrame({'retweet_count': df.groupby(['lang'])['full_text'].count()}).reset_index()
    dfLangCount["language"] = dfLangCount["lang"].astype(str)
    dfLangCount = dfLangCount.sort_values("hashtags", ascending=False)
    dfLangCount.loc[dfLangCount['retweet_count'] < 10, 'lang'] = 'Other languages'
    st.title(" Tweets Language pie chart")
    fig = px.pie(dfLangCount, values='retweet_count', names='language', width=500, height=350)
